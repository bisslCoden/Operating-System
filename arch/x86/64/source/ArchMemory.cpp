...
bool ArchMemory::unmapPage(uint64 virtual_page, ustl::queue<size_t>* ppns)
{
  ...
  assert(checkforPMLCow(virtual_page, ppns, true) && "pmlcow didnt work for some reason...\n");
...
bool ArchMemory::mapPage(uint64 virtual_page, ustl::queue<size_t>* ppns, uint64 user_access)
{
 ...
 // size_t ppn;
  ...
  while (!free_if_fail.empty())
  {
    ppns->push(free_if_fail.front());
    free_if_fail.pop();
  }
