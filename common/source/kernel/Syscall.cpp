...
Syscall::syscallException(size_t syscall_number, size_t arg1, size_t arg2, size_t arg3, size_t arg4, size_t arg5) {
  ...
  {
    debug(SYSCALL,
          "Syscall %zd called with arguments %zd(=%zx) %zd(=%zx) %zd(=%zx) %zd(=%zx) %zd(=%zx) by thread [%ld]\n",
  ...
  if (currentUserThread->getflags()->cancelreq &&
...
size_t Syscall::pthread_create(size_t thread, size_t attr, size_t start_routine, size_t arg, size_t wrapper) {
  debug(SYSCALL,
        "Syscall::pthread_create(thread = %lx, attr = %lx, start_routine = %lx, arg = %lx, wrapper = %lx) called\n",
  ...
  UserThread *newthread = currentUserThread->getParentProcess()->createNewThread(start_routine, arg, wrapper,
                                                                                 joinstate);
...
void Syscall::pthread_exit(void *value) {
  ...
  if (currentUserThread->getProcess()->findInThreadList(my_tid) != 0x00) {
    ...
    if (currentUserThread->getJoiner() != 0) {
      ...
      if ((to_be_signaled = (UserThread *) currentUserThread->getProcess()->findInThreadList(
          currentUserThread->getJoiner()->getTID())) != 0x00) {
        to_be_signaled->signalJoin();
      } else {
        //this will happen if a forked thread calls it
        debug(X_USERTHREAD,
              "Veeery strange! Joiner is not -1 but also not in my process... I must be a forked thread!\n");
      }
    }
    currentUserThread->lockFlagMutex();
...
size_t Syscall::pthread_join(size_t thread, void **value_ptr) {
  ...
  if (currentUserThread->getProcess()->findInThreadList(thread) != 0x00) {
    ...
    currentUserThread->getProcess()->lockRetVal();
    if (join_victim->getJoiner() != 0 || join_victim->getJoiner() == currentUserThread ||
        currentUserThread->detectCircularJoin(join_victim)) {
      debug(X_USERTHREAD,
            "Deadlock in join detected! either thread [%ld] is already joined by another or tries to join each "
...
size_t Syscall::clock() {
  ...
  size_t time_to_add =
      Scheduler::instance()->getRDTSC() / cyc_per_microsec - currentUserThread->getLastStart() / cyc_per_microsec;
