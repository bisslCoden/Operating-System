...
size_t SwapManager::writeToDisk(size_t ppn)
{
  if(lowest_block_ >= max_blocks_)
  {
    assert(false && "No more space on virtual Device\n");
  }
  size_t swapped = lowest_block_;
  used_pages_->setBit(lowest_block_);
  while((lowest_block_ < max_blocks_)&& used_pages_->getBit(lowest_block_))
  {
    ++lowest_block_;
  }
  debug(SWAPMANAGER, "Writing to disk!!\n");
  device_->writeData((swapped * device_->getBlockSize()),device_->getBlockSize(), (char*)ArchMemory::getIdentAddressOfPPN(ppn));
  return swapped;
}

void SwapManager::deleteFromDisk(size_t swap_pn)
{
  //no option to realy delete with device_->... so just remove it from the map
  debug(SWAPMANAGER, "Removing from used map!!\n");
  used_pages_->setBit(swap_pn);
  if(swap_pn < lowest_block_)
  {
    lowest_block_ = swap_pn;
  }
}
