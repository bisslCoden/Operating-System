...
class Syscall
{
  ...
  static int get_pid();
  /**
  Takes the number of clock cycles from now and multiplies by 10 bc of no floating numbers allowed
...
/**
Avg divided by 54925(~54 ms(even though it looks more like 55 ms, but daniel said ~54 if not mistaken))
